...
/* ---------- Event-type config (ported from Settings defaults) ------ */
const EVENT_DEFS = [
...
const DEF_MAP = Object.fromEntries(EVENT_DEFS.map(d => [d.name, d]));

const RELATED_TEAMS = ["Dev", "Design", "Copy", "Legal", "Admin", "QA", "Other"];
const REVISION_TYPES = ["Revision 1", "Revision 2", "Revision 3+", "Graphic Edit", "Copy Edit", "Legal Edit", "QA Fix", "Product Update", "Scope Change", "Other"];
const ENTERED_BY = ["Goun", "Trevin", "Other"];
const PROJECT_TYPES = ["Shogun LP", "Shopify PDP", "Email", "Blog", "Ad Creative", "Website Banner", "Other"];
...
/* ---------- Helpers ------------------------------------------------ */
const waitColor = t => WAIT_COLORS[String(t || "").toLowerCase()] || "#F97316";
const ownerColor = o => OWNER_COLORS[String(o || "").toLowerCase()] || "#6B7280";
const defOf = n => DEF_MAP[n] || { name: n, devState: "Active Dev", defaultTeam: "Dev", bucket: "Active Dev", color: "#999999", rev: false, closes: false, reopens: false };
...
function importEventsCSV(text, existing) {
  ...
  const ci = {
    ts: col(["Timestamp"]), pid: col(["Project ID"]), pname: col(["Project Name"]),
    ev: col(["Dev Event"]), team: col(["Related Team"]),
    task: col(["Task / Change Label", "Task Label"]), rev: col(["Revision / Change Type", "Revision Type"]),
    notes: col(["Notes"]), by: col(["Entered By"]),
  };
  if (ci.ev === -1 || (ci.pid === -1 && ci.pname === -1)) return { error: "Couldn't find required columns (need at least Dev Event and Project ID or Project Name)." };
  ...
  rows.slice(1).forEach((r, idx) => {
    ...
    events.push({
      ...
      relatedTeam: get(r, ci.team), taskLabel: get(r, ci.task), revisionType: get(r, ci.rev),
      notes: get(r, ci.notes), enteredBy: get(r, ci.by),
    });
  });
  const have = new Set(existing.map(p => p.id));
  const newProjects = Object.values(projStub).filter(p => !have.has(p.id));
  return { events, newProjects, skipped, count: events.length };
}
...
function exportEventsCSV(events, projMap) {
  ...
  [...events].sort((a, b) => new Date(a.ts) - new Date(b.ts)).forEach(e => {
    ...
    const team = e.relatedTeam || d.defaultTeam || "";
    lines.push([e.ts, e.projectId, projMap[e.projectId]?.name || "", e.devEvent, d.devState, team, d.bucket, e.taskLabel || "", e.revisionType || "", e.notes || "", e.enteredBy || ""].map(csvCell).join(","));
  });
...
/* ---------- Storage adapter (cloud → artifact → localStorage) ------ */
...
async function cloudGet() {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/tracker?id=eq.${ROW_ID}&select=data`, {
...
const Pill = ({ bg, fg, children }) => (
  <span className="inline-block rounded-full px-2.5 py-0.5 text-xs font-bold" style={{ background: bg, color: fg }}>{children}</span>
);
function dueBadge(s) {
  const map = {
    "overdue": ["#F4CCCC", "#990000"], "due soon": ["#FFF2CC", "#B45F06"], "on track": ["#D9EAD3", "#274E13"],
    "complete": ["#D9EAD3", "#274E13"], "completed late": ["#FCE5CD", "#990000"], "no due date": ["#EFEFEF", "#666"],
  }[String(s).toLowerCase()] || ["#EFEFEF", "#666"];
...
const SectionTitle = ({ children }) => (
  <div className="px-4 py-2.5 text-sm font-bold text-white rounded-t-xl" style={{ background: BLUE }}>{children}</div>
);
...
export default function App() {
  ...
  async function pull() {
    ...
    if (d) { setProjects(d.projects || []); setEvents(d.events || []); }
    else if (!loaded) { setProjects(SEED.projects); setEvents(SEED.events); }
    setSyncing(false);
    setLoaded(true);
  }
  useEffect(() => { pull(); }, []);
  useEffect(() => {
    ...
    saveData({ projects, events });
  }, [projects, events, loaded]);

  const projMap = useMemo(() => Object.fromEntries(projects.map(p => [p.id, p])), [projects]);
  const { segs, byProj } = useMemo(() => buildModel(events, projMap), [events, projMap]);
  const metrics = useMemo(() => projects.map(p => metricsFor(p.id, projMap, segs, byProj)), [projects, projMap, segs, byProj]);
  ...
  const TABS = [
    ...
    ["charts", "Charts", PieIcon], ["flow", "Event Flow", ListOrdered],
    ["projects", "Projects", FolderKanban], ["log", "Event Log", ScrollText],
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900" style={{ fontFamily: "system-ui, sans-serif" }}>
      <div className="text-white" style={{ background: BLUE }}>
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {!READONLY && (
              <button onClick={() => setEventForm({})} className="flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-2 text-sm font-bold" style={{ color: BLUE }}>
      ...
      <div className="mx-auto max-w-7xl px-4 py-5">
        ...
        {tab === "projects" && <Projects metrics={metrics} readOnly={READONLY} onAdd={() => setProjForm({})} onEdit={p => setProjForm(p)}
          onDelete={id => { setProjects(ps => ps.filter(p => p.id !== id)); setEvents(es => es.filter(e => e.projectId !== id)); }} />}
        ...
        {tab === "settings" && <SettingsView readOnly={READONLY} events={events} projMap={projMap}
          onReset={() => { setProjects(SEED.projects); setEvents(SEED.events); }}
      ...
      {eventForm && !READONLY && (
        <EventModal initial={eventForm} projects={projects} onClose={() => setEventForm(null)}
          onSave={ev => { setEvents(es => ev.id ? es.map(e => e.id === ev.id ? ev : e) : [...es, { ...ev, id: "e" + Date.now() }]); setEventForm(null); }} />
      )}
      {projForm && !READONLY && (
        <ProjectModal initial={projForm} existingIds={projects.map(p => p.id)} onClose={() => setProjForm(null)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-blue-50/60"><tr>
              <Th>Project</Th><Th>Due</Th><Th>Status</Th><Th r>Days Here</Th><Th r>Active Dev</Th><Th r>Wait</Th><Th r>Rev</Th><Th r>Reopen</Th><Th>Due Status</Th>
            </tr></thead>
            <tbody className="divide-y divide-gray-100">
              ...
              {active.map((m, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <Td r>{n1(m.currentStateDays)}</Td><Td r>{n1(m.active)}</Td><Td r>{n1(m.wait)}</Td>
                  <Td r>{m.revisions}</Td><Td r>{m.reopens}</Td><Td>{dueBadge(m.dueStatus)}</Td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {done.length > 0 && (
        <Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-blue-50/60"><tr>
                <Th>Project</Th><Th>Completed</Th><Th r>Project Days</Th><Th r>Active Dev</Th><Th r>Wait</Th><Th r>Rev</Th><Th r>Reopen</Th><Th>Due Status</Th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100">
                {done.map((m, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <Td><span className="font-semibold">{m.project.name}</span></Td>
                    <Td>{fmtDT(m.completedDate)}</Td><Td r>{n1(m.daysOpen)}</Td><Td r>{n1(m.active)}</Td><Td r>{n1(m.wait)}</Td>
                    <Td r>{m.revisions}</Td><Td r>{m.reopens}</Td><Td>{dueBadge(m.dueStatus)}</Td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card>
          ...
          <div className="p-4 space-y-2">
            ...
            {teamRows.map(r => {
              const max = teamRows[0].days || 1;
              return (
                <div key={r.team} className="flex items-center gap-2">
                  <div className="w-20 text-sm font-medium">{r.team}</div>
                  <div className="flex-1 h-5 rounded bg-gray-100 overflow-hidden">
                    <div className="h-full rounded" style={{ width: (r.days / max * 100) + "%", background: r.team === "Dev" ? "#2563EB" : waitColor(r.team) }} />
                  </div>
        ...
        <Card>
          ...
          <div className="p-4 space-y-2">
            ...
            {activeByProj.map(r => {
              const max = activeByProj[0].days || 1;
              return (
                <div key={r.name} className="flex items-center gap-2">
                  <div className="w-40 truncate text-sm font-medium" title={r.name}>{r.name}</div>
                  <div className="flex-1 h-5 rounded bg-gray-100 overflow-hidden">
                    <div className="h-full rounded" style={{ width: (r.days / max * 100) + "%", background: "#16A34A" }} />
                  </div>
                  <div className="w-12 text-right text-sm font-bold tabular-nums">{n1(r.days)}d</div>
                </div>
              );
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {revRows.map((m, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <Td><span className="font-semibold">{m.project.name}</span></Td><Td r>{m.revisions}</Td><Td r>{m.reopens}</Td>
                  <Td>{stateBadge(m.currentDevState || m.projectStatus, m.waitingOn)}</Td>
                </tr>
              ))}
            </tbody>
...
function teamTime(segs) {
  const t = {};
  segs.forEach(s => {
    const b = String(s.bucket).toLowerCase();
    if (b !== "active dev" && b !== "waiting") return;
    const team = b === "active dev" ? "Dev" : (s.relatedTeam || "Other");
    t[team] = (t[team] || 0) + (s.durationDays || 0);
  });
  return Object.entries(t).map(([team, days]) => ({ team, days }))
    .sort((a, b) => a.team === "Dev" ? -1 : b.team === "Dev" ? 1 : b.days - a.days);
}
...
function Timeline({ segs, projects }) {
  ...
  const legend = [{ l: "Active Dev", c: "#2563EB" }, ...RELATED_TEAMS.filter(t => t !== "Dev").map(t => ({ l: t, c: waitColor(t) })),
  { l: "Blocked", c: "#6B7280" }, { l: "Dev Complete", c: "#111827" }];
  return (
    <div className="space-y-5">
      ...
      {projects.filter(p => valid.some(s => s.projectId === p.id)).map(p => {
        ...
        const placed = ps.map(s => {
          ...
          let lane = 0;
          while (laneEnd[lane] !== undefined && sc <= laneEnd[lane]) lane++;
          laneEnd[lane] = ec;
          return { s, lane, sc, ec };
        });
            ...
            <div className="overflow-x-auto p-3">
              <div style={{ minWidth: nDays * 48 }}>
                <div style={grid} className="mb-1.5">
                  {days.map((d, i) => <div key={i} className="text-center text-[11px] font-semibold text-gray-500">{(d.getMonth() + 1) + "/" + d.getDate()}</div>)}
                </div>
                {Array.from({ length: lanes }, (_, lane) => (
                  <div key={lane} style={grid} className="mb-0.5">
                    {placed.filter(pl => pl.lane === lane).map((pl, idx) => (
                      <div key={idx} title={`${pl.s.displayStatus}\n${pl.s.taskLabel || ""}\n${n1(pl.s.durationDays)} days${pl.s.notes ? "\n" + pl.s.notes : ""}`}
                        style={{ gridColumn: `${pl.sc + 1} / ${pl.ec + 2}`, background: pl.s.color, minHeight: 34 }}
                        className="rounded flex items-center justify-center px-1 text-center text-[11px] font-bold text-white leading-tight">
...
function Charts({ metrics, segs }) {
  const rows = metrics.filter(m => (m.active + m.wait + m.blocked) > 0)
    .map(m => ({ name: m.project.name, "Active Dev": +m.active.toFixed(2), Waiting: +m.wait.toFixed(2), Blocked: +m.blocked.toFixed(2), total: m.daysOpen || (m.active + m.wait + m.blocked) }))
    .slice(0, 12);
  ...
  const owners = OWNER_ORDER.filter(o => ownerSet[o]);
  const ownerRows = Object.values(ownerData);
  const q = {};
  metrics.forEach(m => {
    const st = m.projectStatus.toLowerCase(), ds = m.currentDevState.toLowerCase();
    let key = "Not Started";
    if (st === "complete" || ds === "dev complete") key = "Complete";
    else if (ds === "waiting" || m.waitingOn) key = "Waiting";
    else if (ds === "blocked") key = "Blocked";
    else if (ds === "active dev") key = "Active Dev";
    q[key] = (q[key] || 0) + 1;
  });
    ...
    <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="p-3" style={{ height: 320 }}>
          <ResponsiveContainer>
            <BarChart data={rows} stackOffset="expand" margin={{ top: 8, right: 8, left: 0, bottom: 30 }}>
              ...
              <Bar dataKey="Active Dev" stackId="a" fill="#2563EB" />
              <Bar dataKey="Waiting" stackId="a" fill="#F97316" />
              <Bar dataKey="Blocked" stackId="a" fill="#111827" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
        ...
        <div className="p-3" style={{ height: 300 }}>
          <ResponsiveContainer>
            <BarChart data={projDays} margin={{ top: 8, right: 8, left: 0, bottom: 30 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 11 }} angle={-20} textAnchor="end" interval={0} height={50} />
              <YAxis tick={{ fontSize: 11 }} /><Tooltip formatter={v => v + "d"} />
      ...
      <Card>
        ...
        <div className="p-3" style={{ height: 300 }}>
          <ResponsiveContainer>
            <PieChart>
              <Pie data={qData} dataKey="value" nameKey="name" innerRadius={55} outerRadius={95} paddingAngle={2}>
                {qData.map((e, i) => <Cell key={i} fill={qColor[e.name] || "#ccc"} />)}
              </Pie>
        ...
        return (
          <Card key={p.id} className="overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-blue-50/60"><tr>
                  <Th>#</Th><Th>Status</Th><Th>Owner</Th><Th>Start</Th><Th>End</Th><Th r>Days</Th><Th>Task / Change</Th><Th>Notes</Th>
                </tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {ps.map((s, i) => (
                    <tr key={i} className="hover:bg-gray-50 align-top">
                      <Td r>{i + 1}</Td>
                      <Td><span className="inline-block rounded px-2 py-0.5 text-xs font-bold text-white" style={{ background: s.color }}>{s.displayStatus}</span></Td>
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-blue-50/60"><tr>
            <Th>ID</Th><Th>Name</Th><Th>Type</Th><Th>Owner</Th><Th>Due</Th><Th>Status</Th><Th>Current State</Th><Th r>Active Dev</Th><Th r>Wait</Th><Th r>Rev</Th><Th>Due Status</Th>{!readOnly && <Th></Th>}
          </tr></thead>
          <tbody className="divide-y divide-gray-100">
            {metrics.length === 0 && <tr><Td><span className="text-gray-400 italic">No projects.</span></Td></tr>}
            {metrics.map((m, i) => (
              <tr key={i} className="hover:bg-gray-50">
                <Td><span className="font-mono text-xs">{m.project.id}</span></Td>
                <Td><span className="font-semibold">{m.project.name}</span></Td>
                <Td>{m.project.type}</Td><Td>{m.project.owner}</Td>
                <Td>{m.project.dueDate ? fmtD(parseDate(m.project.dueDate)) : "—"}</Td>
                ...
                {!readOnly && (
                  <Td>
                    <div className="flex gap-1 justify-end">
                      <button onClick={() => onEdit(m.project)} className="rounded p-1 text-gray-400 hover:text-gray-700 hover:bg-gray-100"><Pencil size={15} /></button>
                      <button onClick={() => { if (confirm("Delete " + m.project.name + " and its events?")) onDelete(m.project.id); }} className="rounded p-1 text-gray-400 hover:text-red-600 hover:bg-red-50"><Trash2 size={15} /></button>
                    </div>
                  </Td>
                )}
              </tr>
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-blue-50/60"><tr>
            <Th>Timestamp</Th><Th>Project</Th><Th>Dev Event</Th><Th>Team</Th><Th>Task / Change</Th><Th>By</Th>{!readOnly && <Th></Th>}
          </tr></thead>
          <tbody className="divide-y divide-gray-100">
            ...
            {rows.map(e => {
              ...
              return (
                <tr key={e.id} className="hover:bg-gray-50">
                  <Td>{fmtDT(parseDate(e.ts))}</Td>
                  <Td>{projMap[e.projectId]?.name || <span className="text-red-500">missing</span>}</Td>
                  ...
                  <Td>{e.relatedTeam || "—"}</Td>
                  <Td>{e.taskLabel}{e.revisionType ? <span className="block text-xs text-gray-500">{e.revisionType}</span> : null}</Td>
                  <Td>{e.enteredBy}</Td>
                  {!readOnly && (
                    <Td>
                      <div className="flex gap-1">
                        <button onClick={() => onEdit(e)} className="rounded p-1 text-gray-400 hover:text-gray-700 hover:bg-gray-100"><Pencil size={15} /></button>
                        <button onClick={() => { if (confirm("Delete this event?")) onDelete(e.id); }} className="rounded p-1 text-gray-400 hover:text-red-600 hover:bg-red-50"><Trash2 size={15} /></button>
                      </div>
                    </Td>
                  )}
                </tr>
...
/* ---------- Settings (config + data tools) ------------------------- */
function SettingsView({ readOnly, events, projMap, onReset, onClear, onImport }) {
  ...
  const List = ({ title, items }) => (
    <Card><SectionTitle>{title}</SectionTitle><div className="p-3 flex flex-wrap gap-1.5">{items.map(i => <span key={i} className="rounded-md bg-gray-100 px-2 py-1 text-sm">{i}</span>)}</div></Card>
  );
  ...
  function onFile(e) {
    ...
    r.onload = () => { setCsv(String(r.result)); doImport(String(r.result)); };
    r.readAsText(f);
  }
  return (
    <div className="space-y-5">
      <Card className="p-4 flex flex-wrap items-center justify-between gap-3">
        <div className="text-sm text-gray-600">
          Storage mode: <b>{STORAGE_MODE === "cloud" ? "Shared (cloud)" : "Local (this browser)"}</b>. Event-type config below is read-only in v1.
        </div>
        {!readOnly && (
          <div className="flex gap-2">
            ...
            <button onClick={() => { if (confirm("Reload illustrative seed data? Replaces current data.")) onReset(); }} className="flex items-center gap-1.5 rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold hover:bg-gray-50"><RefreshCw size={15} /> Reset to Seed</button>
      ...
      {!readOnly && (
        <Card>
          ...
          <div className="p-4 space-y-3">
            <p className="text-sm text-gray-600">Paste your Event Log CSV (or upload a file). Recognized columns: <span className="font-mono text-xs">Timestamp, Project ID, Project Name, Dev Event, Related Team, Task / Change Label, Revision / Change Type, Notes, Entered By</span>. Missing projects are created automatically.</p>
      ...
      <Card className="overflow-hidden">
        <SectionTitle>Dev Event Types</SectionTitle>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-blue-50/60"><tr><Th>Dev Event</Th><Th>Resulting State</Th><Th>Default Team</Th><Th c>Time Bucket</Th><Th c>Color</Th><Th c>Revision</Th><Th c>Closes</Th><Th c>Reopens</Th></tr></thead>
            <tbody className="divide-y divide-gray-100">
              {EVENT_DEFS.map(d => (
                <tr key={d.name} className="hover:bg-gray-50">
                  <Td><span className="font-semibold">{d.name}</span></Td><Td>{d.devState}</Td><Td>{d.defaultTeam || "—"}</Td>
                  <Td c>{d.bucket}</Td><Td c><span className="inline-block h-4 w-8 rounded align-middle" style={{ background: d.color }} /></Td>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <List title="Related Teams" items={RELATED_TEAMS} />
        <List title="Entered By" items={ENTERED_BY} />
        <List title="Revision / Change Types" items={REVISION_TYPES} />
        <List title="Project Types" items={PROJECT_TYPES} />
      </div>
    </div>
  );
}
...
function EventModal({ initial, projects, onClose, onSave }) {
  const editing = !!initial.id;
  const [f, setF] = useState({
    id: initial.id, projectId: initial.projectId || (projects[0]?.id || ""),
    ts: initial.ts || toInput(new Date()), devEvent: initial.devEvent || "",
    relatedTeam: initial.relatedTeam || "", taskLabel: initial.taskLabel || "",
    revisionType: initial.revisionType || "", notes: initial.notes || "", enteredBy: initial.enteredBy || "",
  });
  const [err, setErr] = useState("");
  const def = f.devEvent ? defOf(f.devEvent) : null;
  function pickEvent(name) {
    setF(s => ({ ...s, devEvent: name, relatedTeam: s.relatedTeam || defaultTeamFor(name), taskLabel: s.taskLabel || defaultTaskLabel(name) }));
  }
  function submit() {
    if (!f.projectId || !f.devEvent || !f.taskLabel || !f.enteredBy) { setErr("Project, Dev Event, Task/Change Label, and Entered By are required."); return; }
    if (requiresTeam(f.devEvent) && !f.relatedTeam) { setErr("Related Team is required for this event."); return; }
    onSave({ ...f, ts: typeof f.ts === "string" ? f.ts : toInput(f.ts) });
  }
  return (
    <Modal title={editing ? "Edit Dev Event" : "Log Dev Event"} onClose={onClose}>
      <div className="space-y-3">
        ...
        <Field label="Dev Event" hint={def ? `Creates state: ${def.devState} · Bucket: ${def.bucket}` : "Select what happened — state & clock are derived."}>
          <select className={inputCls} value={f.devEvent} onChange={e => pickEvent(e.target.value)}>
            <option value="">— select —</option>
            {EVENT_DEFS.map(d => <option key={d.name} value={d.name}>{d.name}</option>)}
          </select>
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label={"Related Team" + (def && requiresTeam(f.devEvent) ? " *" : "")}>
            <select className={inputCls} value={f.relatedTeam} onChange={e => setF(s => ({ ...s, relatedTeam: e.target.value }))}>
              <option value="">—</option>{RELATED_TEAMS.map(t => <option key={t}>{t}</option>)}
            </select>
          </Field>
          <Field label="Entered By"><select className={inputCls} value={f.enteredBy} onChange={e => setF(s => ({ ...s, enteredBy: e.target.value }))}>
            <option value="">—</option>{ENTERED_BY.map(t => <option key={t}>{t}</option>)}
          </select></Field>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Revision / Change Type"><select className={inputCls} value={f.revisionType} onChange={e => setF(s => ({ ...s, revisionType: e.target.value }))}>
            <option value="">—</option>{REVISION_TYPES.map(t => <option key={t}>{t}</option>)}
          </select></Field>
...
function ProjectModal({ initial, existingIds, onClose, onSave }) {
  const editing = !!initial.id;
  const [f, setF] = useState({ id: initial.id || "", name: initial.name || "", type: initial.type || PROJECT_TYPES[0], owner: initial.owner || "Dev", startDate: initial.startDate || "", dueDate: initial.dueDate || "" });
  const [err, setErr] = useState("");
  function submit() {
  ...
  return (
    <Modal title={editing ? "Edit Project" : "Add Project"} onClose={onClose}>
      <div className="space-y-3">
        ...
        <Field label="Project Type"><select className={inputCls} value={f.type} onChange={e => setF(s => ({ ...s, type: e.target.value }))}>{PROJECT_TYPES.map(t => <option key={t}>{t}</option>)}</select></Field>
        <div className="grid grid-cols-2 gap-3">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 overflow-y-auto" onClick={onClose}>
      <div className="mt-10 w-full max-w-lg rounded-2xl bg-white shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3.5 text-white rounded-t-2xl" style={{ background: BLUE }}>
          <span className="font-bold">{title}</span>
          <button onClick={onClose} className="rounded-full p-1 hover:bg-white/15"><X size={18} /></button>
        </div>
